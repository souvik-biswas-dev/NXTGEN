...
export default async function DashboardPage() {
  ...
  const statCards = [
    { label: 'Total Users', value: stats.totalUsers, icon: Users, color: 'text-blue-400', bg: 'bg-blue-400/10', href: '/users' },
    { label: 'Total Properties', value: stats.totalProperties, icon: Building2, color: 'text-green-400', bg: 'bg-green-400/10', href: '/properties' },
    { label: 'Active Subscriptions', value: stats.activeSubscriptions, icon: CreditCard, color: 'text-[#FF6B35]', bg: 'bg-[#FF6B35]/10', href: '/subscriptions' },
    { label: 'Total Inquiries', value: stats.totalInquiries, icon: MessageSquare, color: 'text-purple-400', bg: 'bg-purple-400/10', href: '/inquiries' },
    { label: 'Verified Properties', value: stats.verifiedProperties, icon: Star, color: 'text-yellow-400', bg: 'bg-yellow-400/10', href: '/properties' },
    { label: 'Featured Properties', value: stats.featuredProperties, icon: TrendingUp, color: 'text-pink-400', bg: 'bg-pink-400/10', href: '/properties' },
  ];
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
          {statCards.map(({ label, value, icon: Icon, color, bg, href }) => (
            <a key={label} href={href} className="block group">
              <Card className="p-5 transition-colors hover:border-gray-600 group-hover:bg-gray-800/60">
